...
<div class="mengban" style="">
    <div style="width: 1200px;height: 100px;text-align: center;margin: auto;border-bottom: solid 2px #BBBBBB">
        ...
        <div style="float: right;padding-top: 75px;color: #000000;font-size: 14px;height: 30px;margin-right: 50px">
            更新时间：<span id="update_time"></span>
        </div>
    </div>
    <div id="is_show">
        ...
        <div style="width: 1200px;height:auto;text-align: center;margin: auto;margin-top: 15px;">
            <table width="100%" border="1" class="table2" id="baseInfo">
                ...
                <tr style="height: 40px">
                    ...
                    <td>是否已实名认证</td>
                    <td>
                        <span v-if="validateRealName==true">已实名</span>
        ...
        <%--京东白条--%>
        <div style="width: 1200px;height:auto;text-align: center;margin: auto;margin-top: 15px;">
            <table width="100%" border="1" class="table2" id="wealth">
                <tr style="height: 40px;background-color: #0583CE;color: #fff">
                    <td colspan="4">京东白条信息</td>
                </tr>
                <tr style="height: 40px">
                    <td>总额度</td>
                    <td>￥{{creditLimit}}</td>
                ...
                <tr style="height: 40px">
                    <td>是否开通</td>
                    <td>
                        <span v-if="isOpen==true">已开通</span>
                        <span v-else>未开通</span>
                    </td>
                    <td>下月账单最晚还款时间</td>
                    <td>{{nextBillShouldPayLimitDate}}</td>
                </tr>
                <tr style="height: 40px">
                    <td>下月账单最小还款额度</td>
                    <td>￥{{nextBillShouldPayMinAmount}}</td>
                    <td>白条全部代还总金额</td>
        ...
        <%--京东金条--%>
        <div style="width: 1200px;height:auto;text-align: center;margin: auto;margin-top: 15px;">
            <table width="100%" border="1" class="table2" id="wealths">
                <tr style="height: 40px;background-color: #0583CE;color: #fff">
                    <td colspan="4">京东金条信息</td>
                </tr>
                <tr style="height: 40px">
                    <td>总额度</td>
                    <td>￥{{jtCreditLimit}}</td>
                    <td>可用额度</td>
                    <td>￥{{jtAvailableLimit}}</td>
                </tr>
            </table>
        </div>
        <%--收货地址统计--%>
            ...
            <table width="100%" border="1" class="table2" id="order_list">
                ...
                <tr style="height: 40px" v-for="site in sites">
                    ...
                    <td style="width: 360px">
                        <span v-if="site.items[0]==null">-</span>
                        <span v-else>{{site.items[0].productName}}</span>
                    </td>
                    <td>￥{{site.totalPrice}}</td>
                    <td>
                        <span v-if="site.items[0]==null">-</span>
                        <span v-else>{{site.items[0].productCnt}}</span>
                    </td>
                    ...
                    <td>{{site.paymentType}}</td>
                    <td>
                        <span v-if="site.isSuccess==true">交易成功</span>
...
<script>
    ...
    var datainfo = ${dianShangVo.jdData};
    $("#update_time").html(formartTime("${dianShangVo.createTime}"));
    if(datainfo){
        $("#is_hidden").hide();
        $("#is_show").show();
        //基本信息统计
        new Vue({
            el: '#baseInfo',
            data: datainfo.data.basicInfo
        })
        //京东白条
        new Vue({
            el: '#wealth',
            data: datainfo.data.wealth
        })
        //京东金条
        new Vue({
            el: '#wealths',
...
//            收货地址统计
                addressList(data.jinDongItemVos);
//            各个联系人使用统计
                address(data.jinDongItemVoList);
            },"json"
        );
    }else{
        $("#is_hidden").show();
