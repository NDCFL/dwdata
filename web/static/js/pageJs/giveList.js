...
$('#mytab').bootstrapTable({
    ...
    url: "/pcUserGiveItem/pcUserGiveItemList",//要请求数据的文件路径
    ...
    columns: [
        ...
        ,
        {
            ...
            formatter: function (value, row, index) {
                if (value == 0) {
                    //表示启用状态
                    return '<span style="color: green">赠送成功</span>';
                } else {
                    //表示启用状态
                    return '<span style="color: red">赠送失败</span>';
                }
            }
        }
        // ,
        ...
        //     field: '',
        //     formatter: function (value, row, index) {
        //         var e = '<a title="编辑" href="javascript:void(0);" id="pcUserGiveItem"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
...
function del(pcUserGiveItemid, status) {
    if (status == 0) {
    ...
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/pcUserGiveItem/deleteJiFen/' + pcUserGiveItemid,
            dataType: 'json',
...
function edit(name) {
    $.post("/pcUserGiveItem/findJiFen/" + name,
        function (data) {
...
function updatestatus(id, status) {
    $.post("/pcUserGiveItem/updateStatus/" + id + "/" + status,
        function (data) {
            if(status==0){
                if(data.message=="ok"){
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {url: '/pcUserGiveItem/pcUserGiveItemList'});
})
function refush() {
    $('#mytab').bootstrapTable('refresh', {url: '/pcUserGiveItem/pcUserGiveItemList'});
}
$("#update").click(function () {
    $.post(
        "/pcUserGiveItem/pcUserGiveItemUpdateSave",
...
$("#add").click(function () {
    $.post(
        "/pcUserGiveItem/pcUserGiveItemAddSave",
        $("#formadd").serialize(),
        function (data) {
...
function deleteMany11() {
    ...
    layer.confirm('确认要执行批量删除网站信息数据吗？', function (index) {
        $.post(
            "/pcUserGiveItem/deleteManyJiFen",
            {
                "manyId": $("#deleteId").val()
            },
            function (data) {
