...
$('#mytab').bootstrapTable({
    ...
    url: "/apiUserItem/apiUserItemList",//要请求数据的文件路径
    ...
    columns: [
        ...
        ,
        {
            ...
            formatter: function (value, row, index) {
                if (value == 0) {
                    //表示启用状态
                    return '<span style="color: green">查询成功</span>';
                } else {
                    //表示启用状态
                    return '<span style="color: red">查询失败</span>';
                }
            }
        }
        ,
...
function del(apiUserItemid, status) {
    if (status == 0) {
    ...
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/apiUserItem/deleteAPIUserItem/' + apiUserItemid,
            dataType: 'json',
...
function edit(name) {
    $.post("/apiUserItem/findAPIUserItem/" + name,
        function (data) {
...
function updatestatus(id, status) {
    $.post("/apiUserItem/updateStatus/" + id + "/" + status,
        function (data) {
            if(status==0){
                if(data.message=="ok"){
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {url: '/apiUserItem/apiUserItemList'});
})
function refush() {
    $('#mytab').bootstrapTable('refresh', {url: '/apiUserItem/apiUserItemList'});
}
$("#update").click(function () {
    $.post(
        "/apiUserItem/apiUserItemUpdateSave",
...
$("#add").click(function () {
    $.post(
        "/apiUserItem/apiUserItemAddSave",
        $("#formadd").serialize(),
        function (data) {
...
function deleteMany11() {
    ...
    layer.confirm('确认要执行批量删除网站信息数据吗？', function (index) {
        $.post(
            "/apiUserItem/deleteManyAPIUserItem",
            {
                "manyId": $("#deleteId").val()
            },
            function (data) {
